...
fn main() {
    ...
    let mut background_texture = texture_creator.load_texture(Path::new("assets/background.png")).unwrap();
    background_texture.set_alpha_mod(220);
    let carrot_texture = texture_creator.load_texture(Path::new("assets/carrot.png")).unwrap();
    ...
    let mut source_rect = Rect::new(0,
                                    (SPRITE_REST_OFFSET + (SPRITE_SIZE * player_direction) as i32),
                                    SPRITE_SIZE,
                                    SPRITE_SIZE);
    let mut dest_rect = Rect::from_center(Point::new(player_x as i32, player_y as i32),
    ...
    while running {
        ...
        {
            source_rect.set_x(SPRITE_SIZE as i32 * ((ticks / ANIMATION_SPEED) % FRAMES_PER_ANIM));
        }
        else
        {
            // Stop animation
            source_rect.set_x(0);
            source_rect.set_y((SPRITE_REST_OFFSET + (SPRITE_SIZE * player_direction) as i32));
        }
